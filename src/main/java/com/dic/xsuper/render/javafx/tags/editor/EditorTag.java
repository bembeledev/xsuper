...
public class EditorTag extends NativeTag {
    ...
    private List<Map<String, Object>> resolveSyntaxAttribute() {
        ...
        // O DomEvaluator JÁ fez o trabalho duro e injetou os dados crus aqui!
        if (syntaxAttr instanceof List) {
            return (List<Map<String, Object>>) syntaxAttr;
        }

        // Se chegou como String, o dev esqueceu-se dos parêntesis retos no HTML (binding)
        System.err.println("[EditorTag] Aviso de UI: O atributo 'syntax' deve receber uma lista avaliada (use [syntax]=\"variavel\").");
        return null;
    }
        ...
        if (menuIdAttr != null) {
            ...
            editorArea.setOnContextMenuRequested(event -> {
                ...
                if (engine != null && engine.getActiveDom() != null) {
                    XplNode menuVirtualNode = engine.getActiveDom().getElementById(menuId);
