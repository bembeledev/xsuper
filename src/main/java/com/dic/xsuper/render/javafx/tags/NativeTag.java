...
import com.dic.xsuper.render.javafx.cursor.StyleCursorUtils;
import com.dic.xsuper.render.javafx.event.AttachJavaFxListener;
import com.dic.xsuper.dom.event.XplEventType;
import com.dic.xsuper.dom.node.XplNode;
import com.dic.xsuper.render.javafx.layout.panes.CustomLayoutPane;
import com.dic.xsuper.dom.properties.*;
import com.dic.xsuper.dom.properties.borderunit.BorderSide;
import com.dic.xsuper.dom.properties.cssunit.CssContext;
import com.dic.xsuper.dom.properties.gradient.Gradient;
import com.dic.xsuper.dom.properties.gradient.GradientStop;
...
public abstract class NativeTag{
    ...
    public NativeTag(XplNode node) {
        ...
        // ⭐ Tamanho base dinâmico (Lê diretamente do ecrã nativo sem usar Singletons)
        double vw = 800;
        double vh = 600;
        try {
            javafx.geometry.Rectangle2D screenBounds = javafx.stage.Screen.getPrimary().getVisualBounds();
            vw = screenBounds.getWidth();
            vh = screenBounds.getHeight();
    ...
    public Node build() {
        ...
        com.dic.xsuper.render.javafx.helpers.StyleTransformUtils.applyTransforms(fxNode, new HashMap<>(this.style));
        addChildren();
        bindEvents();
        ...
        // ==========================================================
        // ⭐ 4. O GATILHO DAS ANIMAÇÕES (Context Provider Pattern)
        // ==========================================================
        if (styles.containsKey("animation")) {
            ...
            if (parts.length > 0) {
                ...
                // Em vez de chamar o Singleton global, esperamos que a tag seja colada na Janela.
                if (fxNode.getScene() != null) {
                    playAnimationSafely(fxNode, animName, overrides);
                } else {
    ...
    private void playAnimationSafely(Node node, String animName, Map<String, String> overrides) {
        // A janela (Scene) já existe. Vamos ao topo da árvore pedir o Gestor!
        Object managerObj = node.getScene().getRoot().getProperties().get("xpl_animation_manager");

        if (managerObj instanceof com.dic.xsuper.render.javafx.animation.XplAnimationManager manager) {
            com.dic.xsuper.render.javafx.animation.XplAnimationEngine.playAnimation(
                    node, animName, overrides, manager
            );
        } else {
            System.err.println("[NativeTag] Aviso: AnimationManager não encontrado na raiz da janela.");
        }
    }
    ...
    protected void applyCommonStyles() {
        ...
        W3cCssAdapter.applyW3cToNative(fxNode, style, css);
    ...
    protected void bindContextMenu() {
        ...
        if (menuIdAttr != null) {
            ...
            fxNode.setOnContextMenuRequested(event -> {
                ...
                // 1. Procura a tag <contextmenu> em qualquer parte da memória usando o Radar Local
                XplNode menuVirtualNode = findNodeByIdLocal(menuId);

                if (menuVirtualNode != null && menuVirtualNode.nativeNode instanceof javafx.scene.Node dummyNode) {
                    // 2. Lê o menu que a ContextMenuTag guardou em segredo!
                    ...
                    if (rawMenu instanceof javafx.scene.control.ContextMenu ctxMenu) {
                        // 3. Exibe o menu exatamente na coordenada (X,Y) do ecrã onde o rato clicou
                        ctxMenu.show(fxNode, event.getScreenX(), event.getScreenY());
                    }
                } else {
                    System.err.println("[NativeTag] Aviso: O menu de contexto com id '" + menuId + "' não foi encontrado no DOM local.");
                }
            });
        }
    }
    ...
    // =========================================================================
    //   RADAR LOCAL (Adeus Singletons Globais!)
    // =========================================================================
    protected XplNode findNodeByIdLocal(String targetId) {
        if (sourceNode == null || targetId == null) return null;

        // 1. Sobe até à raiz da janela atual
        XplNode root = sourceNode;
        while (root.parent != null) {
            root = root.parent;
        }

        // 2. Desce à procura do ID apenas nesta janela
        return searchById(root, targetId);
    }

    private XplNode searchById(XplNode node, String id) {
        if (id.equals(node.id)) return node;
        for (XplNode child : node.children) {
            XplNode found = searchById(child, id);
