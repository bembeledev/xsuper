...
import com.dic.xsuper.render.javafx.animation.XplTransition;
...
public class JavaFxRenderer implements XplUiBridge {
    ...
    private void patchNodeProperties(XplNode oldNode, XplNode newNode, Node fxNode) {
        ...
        if (!Objects.equals(oldNode.textContent, newNode.textContent)) {
            ...
            } else if (fxNode instanceof javafx.scene.text.Text textObj) {
                // ⭐ A CURA DO TEMPO: O VDOM agora sabe atualizar os nós de texto puro!
                textObj.setText(newNode.textContent != null ? newNode.textContent : "");
            }
        }
        ...
        for (Map.Entry<String, String> newEntry : newNode.style.entrySet()) {
            ...
            if (!java.util.Objects.equals(oldVal, newVal)) {
                ...
                if (transitionConfig != null && (transitionConfig.contains(prop) || transitionConfig.contains("all"))) {
                    ...
                    com.dic.xsuper.render.javafx.animation.XplAnimationEngine.applyTransition(
                            fxNode, prop, physicalOldVal, newVal,
                            new com.dic.xsuper.render.javafx.animation.XplTransition(transitionConfig)
                    );
                } else {
        ...
        switch (prop) {
            ...
            case "text", "textcontent", "innerhtml" -> {
                if (fxNode instanceof javafx.scene.control.Labeled labeled) {
                    ...
                    // ⭐ A CURA DA REATIVIDADE CIRÚRGICA!
                    textObj.setText(valStr);
    ...
    private double parseSize(String value) {
        ...
        try {
            ...
            if (isVh && SuperUiEngine.getInstance() != null) {
                return (val / 100.0) * SuperUiEngine.getInstance().getViewportHeight();
            }
            if (isVw && SuperUiEngine.getInstance() != null) {
                return (val / 100.0) * SuperUiEngine.getInstance().getViewportWidth();
            }
