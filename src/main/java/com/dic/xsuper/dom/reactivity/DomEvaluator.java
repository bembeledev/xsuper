   ...
   public DomEvaluator(Interpreter interpreter, SuperUiEngine engine, XplReactiveState reactiveState) {
    ...
    private List<XplNode> evaluateForBlock(XplNode forNode) {
        ...
        if (!items.isEmpty()) {
            for (Object item : items) {
                ...
                Environment shadowScope = new Environment(interpreter.environment);
    ...
    public Object evaluateExpressionXPL(String expressao) {
        ...
        try {
            ...
            Lexer lexer = new Lexer(codigoInjetado, "DOM_Binding");
            ...
            if (!statements.isEmpty()) {
                Stmt primeiroStmt = statements.getFirst();

                if (primeiroStmt instanceof Stmt.ExpressionStmt exprStmt) {
                    return interpreter.evaluate(exprStmt.expression);
                } else if (primeiroStmt instanceof Stmt.Return retStmt) {
