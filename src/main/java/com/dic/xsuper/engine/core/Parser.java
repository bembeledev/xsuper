...
public class Parser {
    ...
    private Stmt enumDeclaration() {
        Token name = consume(TokenType.IDENTIFIER, "Esperava o nome do Enum.");
        consume(TokenType.LBRACE, "Esperava '{' antes do corpo do Enum.");

        List<Stmt.EnumVariant> variants = new ArrayList<>();

        while (!check(TokenType.RBRACE) && !isAtEnd()) {
            Token variantName = consume(TokenType.IDENTIFIER, "Esperava o nome da variante do Enum.");
            List<Token> params = new ArrayList<>();

            // Se tiver '(' significa que é uma variante que guarda dados (Estilo Rust)
            if (match(TokenType.LPAREN)) {
                if (!check(TokenType.RPAREN)) {
                    do {
                        params.add(consume(TokenType.IDENTIFIER, "Esperava o nome do parâmetro da variante."));
                    } while (match(TokenType.COMMA));
                }
                consume(TokenType.RPAREN, "Esperava ')' após os parâmetros da variante.");
            }

            variants.add(new Stmt.EnumVariant(variantName, params));

            // A vírgula entre variantes é opcional/obrigatória dependendo de como queres
            match(TokenType.COMMA);
        }

        consume(TokenType.RBRACE, "Esperava '}' após o corpo do Enum.");
    ...
    private Stmt declaration() {
        ...
        try {
            ...
            if (isSealed || match(TokenType.DECLARE)) {
                ...
                // ⭐ CORREÇÃO 1: Adicionado o 'decorators' aqui!
            ...
            if (match(TokenType.ABSTRACT)) {
                ...
                // ⭐ CORREÇÃO 2: Adicionado o 'decorators' aqui!
                return implementDeclaration(true, decorators, listeners);
            }

            if (match(TokenType.IMPLEMENT)) {
                // ⭐ A NOVA ROTA! Se a seguir a 'implement' vier 'listener'...
                if (match(TokenType.LISTENER)) {
                    return implementListenerStmt();
                }
                // ⭐ CORREÇÃO 3: Adicionado o 'decorators' e corrigido isAbstract para 'false'!
    ...
    // ⭐ CONSTRUTOR DO BLUEPRINT DO LISTENER (Apenas Estado) ⭐
    private Stmt listenerDeclaration() {
        ...
        while (!check(TokenType.RBRACE) && !isAtEnd()) {
            ...
            // Um listener puro agora SÓ declara propriedades de estado!
        ...
        // ⭐ CORRIGIDO: Retorna o novo nó isolado ListenerDecl!
        return new Stmt.ListenerDecl(name, fields);
    }

    // ⭐ A NOVA ROTA: implement listener Nome { ... } ⭐
    private Stmt implementListenerStmt() {
        Token targetName = consumeIdentifierSoft("Esperado o nome do listener alvo.");
        consumeSoft(TokenType.LBRACE, "{", "Esperado '{' antes dos métodos do listener.");

        java.util.List<Stmt.Function> methods = new java.util.ArrayList<>();

        while (!check(TokenType.RBRACE) && !isAtEnd()) {
            // 1. Mochilas (Metadados como @(Listen.Set))
            ...
            // 2. Modificadores
            Token accessModifier = null;
            boolean isStatic = false, isAbstract = false;
            ...
            consumeSoft(TokenType.FUN, "fun", "Esperada a palavra 'fun' para declarar um método.");
            ...
            if (!check(TokenType.RPAREN)) {
                do {
                    boolean isRest = match(TokenType.SPREAD);
                    Token paramName = consumeIdentifierSoft("Esperado nome do parâmetro.");
                    consumeSoft(TokenType.COLON, ":", "Esperado ':'.");
                    TypeNode type = parseTypeAnnotation();
                    parameters.add(new Stmt.Param(paramName, type, null, isRest));

                    if (isRest && check(TokenType.COMMA)) {
                        throw error(peek(), "O parâmetro Rest '...' tem de ser obrigatoriamente o último.");
                    }
                } while (match(TokenType.COMMA));
            }
            ...
            if (match(TokenType.THROWS)) {
                do {
                    thrownExceptions.add(consumeIdentifierSoft("Esperado nome da exceção."));
                } while (match(TokenType.COMMA));
            }

            java.util.List<Stmt> body = null;
            if (isAbstract) {
                consumeSoft(TokenType.SEMICOLON, ";", "Esperado ';' após a assinatura abstrata.");
            } else {
                consumeSoft(TokenType.LBRACE, "{", "Esperado '{' antes do corpo.");
                body = block();
            }

            methods.add(new Stmt.Function(accessModifier, isStatic, isAbstract, methodName, parameters, returnType, thrownExceptions, body, methodDecorators, new java.util.ArrayList<>()));
        }

        consumeSoft(TokenType.RBRACE, "}", "Esperado '}' após os métodos do listener.");
        return new Stmt.ImplementListener(targetName, methods);
    }
        ...
        while (!check(TokenType.RBRACE) && !isAtEnd()) {
            ...
            if (!check(TokenType.RPAREN)) {
                do {
                    ...
                    // ⭐ DETETOR DE REST PARAMETERS (ex: ...numeros)
                    boolean isRest = match(TokenType.SPREAD);
                    ...
                    if (match(TokenType.ASSIGN)) {
                        if (isRest) throw error(previous(), "O parâmetro Rest '...' não pode ter um valor por defeito.");
                        defaultValue = expression();
                    }

                    // ⭐ ATUALIZADO: Passamos o 'isRest'
                    parameters.add(new Stmt.Param(paramName, type, defaultValue, isRest));

                    // Validação sintática: o '...' tem de ser o último!
                    if (isRest && check(TokenType.COMMA)) {
                        throw error(peek(), "O parâmetro Rest '...' tem de ser obrigatoriamente o último parâmetro da função.");
                    }
        ...
        while (!check(TokenType.RBRACE) && !isAtEnd()) {
            ...
            if (!check(TokenType.RPAREN)) {
                do {
                    ...
                    // ⭐ DETETOR DE REST PARAMETERS (ex: ...args)
                    boolean isRest = match(TokenType.SPREAD);
                    ...
                    // ⭐ ATUALIZADO: Passamos o 'isRest' como último argumento
                    parameters.add(new Stmt.Param(paramName, type, defaultValue, isRest));

                    // Validação sintática rigorosa: o '...' tem de ser o último parâmetro da lista!
                    if (isRest && check(TokenType.COMMA)) {
                        throw error(peek(), "O parâmetro Rest '...' tem de ser obrigatoriamente o último parâmetro da função.");
                    }
    ...
    private Stmt functionDeclaration(java.util.List<Stmt.DecoratorNode> decorators, java.util.List<Stmt.DecoratorNode> listeners) {
        ...
        if (!check(TokenType.RPAREN)) {
            do {
                ...
                // Validação sintática rigorosa: o '...' tem de ser o último parâmetro!
                if (isRest && check(TokenType.COMMA)) {
                    throw error(peek(), "O parâmetro Rest '...' tem de ser obrigatoriamente o último parâmetro da função.");
                }
...
// =========================================================================
    ...
    private Expr objectLiteral() {
        ...
        if (!check(TokenType.RBRACE)) {
            do {
                // ⭐ DETETOR DE SPREAD NO OBJETO!
                if (match(TokenType.SPREAD)) {
                    Token spreadToken = previous();
                    Expr spreadTarget = expression();
                    // Colocamos key como NULL para o Interpretador saber que isto é um Spread e não um par chave-valor normal!
                    keys.add(null);
                    values.add(new Expr.Spread(spreadToken, spreadTarget));
                } else {
                    Expr key;
    ...
    private Expr assignment() {
        ...
        if (check(TokenType.LPAREN)) {
            ...
            if (isArrow) {
                ...
                if (!check(TokenType.RPAREN)) {
                    do {
                        // ⭐ DETETOR DE REST PARAMETERS NA ARROW FUNCTION
                        boolean isRest = match(TokenType.SPREAD);
                        ...
                        // ⭐ ATUALIZADO: Passamos o 'isRest' para o construtor
                        arrowParams.add(new Stmt.Param(paramName, type, null, isRest));
                    } while (match(TokenType.COMMA));
                }
        ...
        if (check(TokenType.IDENTIFIER) && current + 1 < tokens.size() && tokens.get(current + 1).type == TokenType.FAT_ARROW) {
            ...
            singleParam.add(new Stmt.Param(paramName, null, null,false));
    ...
    private Expr factor() {
        ...
        while (match(TokenType.STAR, TokenType.SLASH, TokenType.MODULO, TokenType.HASH)) {
            Token operator = previous();
    ...
    // ⭐ CONSTRUTOR ESTRUTURAL DO DECORADOR (Estilo @Annotation) ⭐
    private Stmt decoratorDeclaration() {
        ...
        // ⭐ CORRIGIDO: Só 2 argumentos. Decoradores não têm métodos na AST!
    ...
    private Expr finishCall(Expr callee) {
        ...
        if (!check(TokenType.RPAREN)) {
            do {
                // ⭐ DETETOR DE SPREAD NOS ARGUMENTOS!
                if (match(TokenType.SPREAD)) {
                    Token spreadToken = previous();
                    Expr spreadTarget = expression();
                    arguments.add(new Expr.CallArg(null, new Expr.Spread(spreadToken, spreadTarget)));
                } else {
                    Token argName = null;
                    if (check(TokenType.IDENTIFIER) && peekNext().type == TokenType.COLON) {
                        argName = consumeIdentifierSoft( "Esperado identificador do argumento.");
    ...
    private Expr primary() {
        ...
        if (match(TokenType.LBRACKET)) {
            // Array Literal ex: [1, 2, ...outraLista, 3]
            List<Expr> elements = new ArrayList<>();
            if (!check(TokenType.RBRACKET)) {
                do {
                    // ⭐ DETETOR DE SPREAD NO ARRAY!
                    if (match(TokenType.SPREAD)) {
                        Token spreadToken = previous();
                        Expr spreadTarget = expression();
                        elements.add(new Expr.Spread(spreadToken, spreadTarget));
                    } else {
