...
public class BubbleChartTag extends ChartBase {

    private String symbolType = "circle";

    public BubbleChartTag(XplNode node) {
        super(node);
    }
    ...
    protected void populateData(Object data) {
        if (!(activeChart instanceof BubbleChart)) return;
        BubbleChart<Number, Number> chart = (BubbleChart<Number, Number>) activeChart;
        chart.getData().clear();
            ...
            if (valuesObj instanceof List) {
                for (Object item : (List<?>) valuesObj) {
                    if (item instanceof Map) {
                        ...
                        double radius = toDouble(point.getOrDefault("radius", 5));
                        XYChart.Data<Number, Number> dataPoint = new XYChart.Data<>(x, y);
                        dataPoint.setExtraValue(radius);
