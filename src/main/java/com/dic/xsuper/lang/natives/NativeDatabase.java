...
/**
 * Módulo Nativo para Bases de Dados via JDBC.
 * Suporta PostgreSQL, MySQL, SQLite, H2, Oracle, etc.
 * <p>
 * Uso:
 *   var conn = db_connect("jdbc:postgresql://localhost:5432/meu_db", "user", "pass");
 *   var rows = conn.query("SELECT * FROM users WHERE age > ?", [18]);
...
public class NativeDatabase {
    ...
    public static void register(Interpreter interpreter) {
        ...
        // ─── db_connect(url, [user, password]) ──────────────────────────────
        interpreter.globals.defineConst("db_connect", new XplCallable() {
            @Override public int arity() { return -1; } // variável
            ...
            public Object call(Interpreter intp, List<Expr.CallArg> args) {
                ...
                if (evaluated.isEmpty()) {
                    throw new ControlFlow.RuntimeError(null, "db_connect: precisa pelo menos da URL.");
                }
                ...
                // Carrega driver automaticamente
        ...
        // ─── db_driver(class) ──────────────────────────────────────────────
        interpreter.globals.defineConst("db_driver", buildFunc(1, (intp, args) -> {
            ...
            try {
                ...
                throw new ControlFlow.RuntimeError(null,
                        "Driver não encontrado: " + driverClass);
            }
        }));
    }
        ...
        public Object invokeMethod(String methodName, List<Object> args, Interpreter interpreter) {
            ...
            try {
                switch (methodName) {
                    ...
                    default:
                        throw new ControlFlow.RuntimeError(null,
                                "Método '" + methodName + "' não suportado para ligação.");
                }
            } catch (SQLException e) {
        ...
        private List<Map<String, Object>> resultSetToList(ResultSet rs) throws SQLException {
            ...
            while (rs.next()) {
                ...
                for (int i = 1; i <= columnCount; i++) {
                    ...
                    // Conversão de tipos para simplificar no XPL
        ...
        public Object invokeMethod(String methodName, List<Object> args, Interpreter interpreter) {
            ...
            try {
                switch (methodName) {
                    ...
                    default:
                        throw new ControlFlow.RuntimeError(null,
                                "Método '" + methodName + "' não suportado no PreparedStatement.");
                }
            } catch (SQLException e) {
                throw new ControlFlow.RuntimeError(null, "Erro de SQL: " + e.getMessage());
            }
    ...
    private static void loadDriverForUrl(String url) {
        ...
        try {
            ...
            // Não há driver registado; o utilizador terá de chamar db_driver
        }
    }
